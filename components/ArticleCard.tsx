...
export default function ArticleCard({ article: a, size = 'md' }: { article: Article, size?: 'sm'|'md'|'lg' }) {
  ...
  if (size === 'lg') return (
    <Link href={`/article/${a.id}`} className="group block relative overflow-hidden rounded-2xl h-[480px] bg-gray-900">
      <Image src={img} alt={a.title} fill className="object-cover opacity-70 group-hover:opacity-80 group-hover:scale-105 transition-all duration-500 card-img" onError={(e) => { (e.target as any).src = brandFallbackImg(a.brand) }}/>
  ...
  if (size === 'sm') return (
    <Link href={`/article/${a.id}`} className="group flex gap-3 items-start hover:bg-gray-50 rounded-xl p-2 -mx-2 transition-colors">
      <div className="relative w-20 h-16 rounded-lg overflow-hidden flex-shrink-0">
        <Image src={img} alt={a.title} fill className="object-cover group-hover:scale-105 transition-transform duration-300" onError={(e) => { (e.target as any).src = brandFallbackImg(a.brand) }}/>
      </div>
      <div className="flex-1 min-w-0">
        <span className="text-xs font-bold px-2 py-0.5 rounded-full text-white inline-block mb-1" style={{background: color, fontSize: '10px'}}>{label}</span>
  ...
  return (
    <Link href={`/article/${a.id}`} className="group card block bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-0.5">
      <div className="relative overflow-hidden" style={{aspectRatio:'16/9'}}>
        <Image src={img} alt={a.title} fill className="object-cover card-img" onError={(e) => { (e.target as any).src = brandFallbackImg(a.brand) }}/>
