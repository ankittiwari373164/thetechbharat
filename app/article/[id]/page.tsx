      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <article className="lg:col-span-2">
          ...
          <div className="flex items-center gap-3 mb-4 flex-wrap">
            ...
            {article!.views > 0 && (
              <><span className="text-gray-300">·</span><span className="text-sm text-gray-500">{article!.views.toLocaleString()} views</span></>
            )}
          </div>
            ...
            <div className="flex items-center gap-3 mb-5 bg-yellow-50 border border-yellow-200 rounded-xl p-4">
              ...
              <div>
                <div className="flex gap-0.5">
                  {[1,2,3,4,5].map(s => (
                    <span key={s} className={`text-xl ${s <= Math.round(article!.rating as number) ? 'text-yellow-400' : 'text-gray-200'}`}>★</span>
                  ))}
                </div>
          ...
          {intro && (
            <p className="text-lg text-gray-700 leading-relaxed font-medium mb-6 border-l-4 border-red-600 pl-4 bg-red-50 py-3 rounded-r-xl">
              {intro}
            </p>
          )}
          ...
          <div className="mt-8 flex items-center gap-3 flex-wrap">
            ...
            <a href={`https://wa.me/?text=${encodeURIComponent(article!.title + ' ' + shareUrl)}`}
              target="_blank" rel="noopener"
              className="bg-green-500 text-white px-4 py-2 rounded-full text-sm font-bold hover:bg-green-600 transition-colors">
              WhatsApp
            </a>
            <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(article!.title)}&url=${encodeURIComponent(shareUrl)}`}
              target="_blank" rel="noopener"
              className="bg-sky-500 text-white px-4 py-2 rounded-full text-sm font-bold hover:bg-sky-600 transition-colors">
              Twitter/X
            </a>
          </div>
        </article>
            ...
            <div className="space-y-3">
              {trending.map((t, i) => (
                <Link key={t.id} href={`/article/${t.id}`}
                  className="flex gap-3 hover:bg-gray-50 rounded-xl p-2 -mx-2 transition-colors group">
                  <span className="text-2xl font-black text-gray-200 w-7 flex-shrink-0 leading-none">{i + 1}</span>
                  <span className="text-sm font-semibold text-gray-800 group-hover:text-red-600 line-clamp-2 transition-colors">{t.title}</span>
                </Link>
              ))}
            </div>
          </div>
          <div className="bg-gray-100 rounded-2xl flex items-center justify-center h-60 text-sm text-gray-400 border-2 border-dashed border-gray-200">
            Advertisement
          </div>
        </aside>
      </div>
      ...
      {similar.length > 0 && (
        <section className="mt-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {similar.map(s => <ArticleCard key={s.id} article={s} />)}
          </div>
        </section>
      )}
    </div>
